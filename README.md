...
## What Was Reused / Removed

Reused: `Application` fields + DataAnnotations + `IValidatableObject` rules,
`ApplicationDbContext` entity configuration, CRUD query patterns
(`OrderByDescending(Id)`, `FindAsync`, `SaveChangesAsync`), seed records,
and the testing intent (xUnit + Moq, isolated DB, HTTP integration tests).

Removed: `Views/` + all `.cshtml`, `wwwroot/`, `ErrorController`,
`AddControllersWithViews()`, `return View(...)`, `UseSqlite()`,
`Microsoft.EntityFrameworkCore.Sqlite`, SQLite connection strings,
`Data/placement_tracker.db`, and the old MVC test project.
